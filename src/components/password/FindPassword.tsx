...
const FindPassword = ():JSX.Element =>{
    ...
    const { requestEmailVerification } = useMemberPw()
    
    const handleSubmit = () =>{
        if (email) {
            requestEmailVerification(email);  // 이메일을 서버로 전송
    ...
    return(
        <div className="text-center w-[400px] mx-auto">
            ...
            <InputField type="email" id="email" name="email" value={email}
                placeholder="이메일을 입력해주세요." className="text-[13px] mb-6" onChange={(e) => setEmail(e.target.value)}  
            />
