...
const Calendar = ({ onDateSelect, startDate, endDate, className }:CalendarProps):JSX.Element =>{
    ...
    const generateCalendar = () =>{
        ...
        for (let i = 1; i <= lastDate; i++) {
            ...
            days.push(
                <div key={i} className={`text-[14px] text-center py-2 md:py-3 lg:py-4 cursor-pointer 
    ...
    return(
        <div className={`${className} mx-auto border-2 border-gray5 rounded-[5px] drop-shadow-lg bg-white py-6 px-2`}>
            <div className="flex justify-center items-center">
                <button onClick={prevMonth} className="p-2 text-myBlue"><IoIosArrowBack /></button>
                <h2 className="nexon-medium text-black3 mx-5 text-[14px]">{`${currentYear}년 ${currentMonth+1}월`}</h2>
                <button onClick={nextMonth} className="p-2 text-myBlue"><IoIosArrowForward /></button>
            </div>
            <div className="grid grid-cols-7 gap-1 px-14 py-4 md:px-1 lg:px-20 lg:py-6">
                {
                    date.map((d)=>(
                        <div key={d} className={`text-center text-[14px] ${d === "Su" ? "text-red-500" : "text-black8"}`}>{d}</div>
                    ))
                }
                {generateCalendar()}
            </div>
