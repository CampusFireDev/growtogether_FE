...
const StudySchedule = ({ scheduleId, title, start, totalTime, author }: { scheduleId: number; title: string; start: string; totalTime: number; author: string }): JSX.Element => {
    // 일정 수정 팝업 상태 관리
    ...
    const handleDelete = async () => {
        if (confirm("일정을 삭제하시겠습니까?")) {
            try {
                await trigger(`https://www.growtogether.store/study/schedule/${scheduleId}`);
                alert("일정이 삭제되었습니다.");
    ...
    return (
        <li className="flex items-center justify-between w-full px-[25px] py-[25px] bg-white9 rounded-[5px]">
            ...
            <div className="flex items-center gap-4">
                <button className="flex items-center gap-1 text-sm text-black6 cursor-pointer" onClick={() => setIsEditOpen(true)}>
